...
// ---------------------------
// 1️⃣ Back link with type param
// ---------------------------
const backLink = document.getElementById("back-link");
const typeParam = new URLSearchParams(window.location.search).get("type");
if (typeParam) {
  backLink.href = `projects.html?type=${encodeURIComponent(typeParam)}`;
} else {
...
const layoutProjects = [
  ...
  },
  {
    ...
    desc: "Project NARAYANA ABODE Phase-2 Resides Near The Mysore Express Way very nearly At Kambipura with Luxurious Surrounded by Rapid Dovelopment. BDA Approved Layout, Loans Available from both Private And Public Sector Banks. Spreading Across 11 Acres. Well Planned And Smartly Developed Gated Community in Bangalore South And Just 12Km From Town Hall.",
    location: "Kambipura",
    details:
...
let project;
if (type === "apartment") project = apartmentProjects[id];
else if (type === "layout") project = layoutProjects[id];

// ---------------------------
// 4️⃣ Change Background Based on Type
// ---------------------------
...
if (project) {
  ...
  let attributesHtml = "";
  if (type === "apartment") {
    attributesHtml += project.Units
      ? `<div><strong>Units</strong> ${project.Units}</div>`
      : "";
    attributesHtml += project.floors
      ? `<div><strong>Floors</strong> ${project.floors}</div>`
      : "";
  }
  attributesHtml += project.plotSize
    ? `<div><strong>Plot Size</strong> ${project.plotSize}</div>`
    : "";
  attributesHtml += project.totalLand
    ? `<div><strong>Total Land</strong> ${project.totalLand}</div>`
    : "";
  if (project.amenities) {
    attributesHtml += `<div><strong>Amenities</strong><ul>${project.amenities
      .map((a) => `<li>${a}</li>`)
      .join("")}</ul></div>`;
  }
  document.getElementById("project-detail").innerHTML = `
        ...
        <p>${project.desc}</p>
        <div class="project-full-details">${project.details}</div>
  ...
  // Slideshow
  const slideshowContainer = document.getElementById("simple-slideshow");

  // Remove previous slides
  slideshowContainer
    .querySelectorAll(".simple-slide-img")
  ...
  project.images.forEach((src, index) => {
    ...
    img.className = "simple-slide-img";
    if (index === 0) img.classList.add("active");
    slideshowContainer.appendChild(img);
  });

  // Slide navigation
  const slides = document.querySelectorAll(".simple-slide-img");
  let current = 0;
  ...
  document.getElementById("slide-prev").onclick = () => showSlide(current - 1);
  document.getElementById("slide-next").onclick = () => showSlide(current + 1);
} else {
